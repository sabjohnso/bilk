...
let test_client_sends_eval () =
  ...
  Alcotest.(check bool) "eval" true
    (match msg with Repl_protocol.Eval s -> s = "abc" | _ -> false);
...
let test_client_receives_result () =
  let (client_fd, server_fd) = Unix.socketpair Unix.PF_UNIX Unix.SOCK_STREAM 0 in
  let buf = Buffer.create 64 in
  Repl_protocol.write_server_msg buf (Result "3");
  let data = Buffer.contents buf in
  let _ = Unix.write_substring server_fd data 0 (String.length data) in
  let recv = Bytes.create 256 in
  let n = Unix.read client_fd recv 0 256 in
  ...
  Alcotest.(check bool) "result" true
    (match msg with Repl_protocol.Result s -> s = "3" | _ -> false);
  Unix.close client_fd;
  Unix.close server_fd
...
let () =
  Alcotest.run "Repl_client"
    [ ("protocol",
       [ Alcotest.test_case "send eval" `Quick test_client_sends_eval
       ; Alcotest.test_case "receive output" `Quick test_client_receives_output
       ; Alcotest.test_case "receive result" `Quick test_client_receives_result
       ])
    ; ("config",
