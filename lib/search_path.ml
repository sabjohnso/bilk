...
let bilk_home () =
  ...
  | _ ->
    ...
    | Some home -> Filename.concat home ".bilk"
    | None -> ".bilk"

let site_lib () =
  Filename.concat (bilk_home ()) "lib"

let env_paths () =
  match Sys.getenv_opt "BILK_PATH" with
  | None | Some "" -> []
  | Some s ->
...
let venv_lib_path () =
  match Sys.getenv_opt "BILK_VENV" with
  | None | Some "" -> None
  | Some dir ->
...
let discover_installed_stdlib () =
  try
    ...
    let path = List.fold_left Filename.concat prefix ["share"; "bilk"; "stdlib"] in
    if dir_exists path then [path] else []
  with _ -> []

let stdlib_dirs () =
  match Sys.getenv_opt "BILK_STDLIB" with
  | Some dir when dir <> "" ->
    if dir_exists dir then [dir] else []
  | _ ->
    if dir_exists Bilk_config.stdlib_source_dir then
      [Bilk_config.stdlib_source_dir]
    else
