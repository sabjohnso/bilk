...
let handle_server_msg msg =
  ...
  | Repl_protocol.Result s ->
    let _ = Unix.write_substring Unix.stdout s 0 (String.length s) in
    let _ = Unix.write_substring Unix.stdout "\n" 0 1 in
    true
  | Repl_protocol.Error s ->
    let _ = Unix.write_substring Unix.stderr s 0 (String.length s) in
    let _ = Unix.write_substring Unix.stderr "\n" 0 1 in
    true
  | Repl_protocol.Completions _ -> true
  | Repl_protocol.Read_request _ -> true
  | Repl_protocol.Status _ -> true
  | Repl_protocol.Session_ok -> true
  | Repl_protocol.Session_deny -> false
    ...
    List.iter (fun fd ->
      if fd == Unix.stdin then begin
        ...
        if n > 0 then begin
          ...
          (try send_msg sock (Repl_protocol.Eval s)
