...
*** TODO Issue 64: No max frame size in protocol
:PROPERTIES:
:PRIORITY: A
:CATEGORY: security
:END:
...
**** Plan
- Add ~max_frame_size~ constant (e.g. 16 MiB)
- Reject frames exceeding ~max_frame_size~ in ~read_client_msg~ /
  ~read_server_msg~ with a protocol error
- Guard ~frame_available~ against integer overflow: check
  ~payload_len > max_frame_size~ before arithmetic
- Write tests: oversized frame header rejected, boundary values handled
...
*** TODO Issue 66: No server authentication (MITM possible)
:PROPERTIES:
:PRIORITY: A
:CATEGORY: security
:END:
...
**** Plan
- Implement server identity verification: e.g. server sends a challenge
  signed with the shared key, or adopt a proper key exchange (Noise
  protocol, STS) instead of pre-shared key
- At minimum, add TOFU (trust-on-first-use): store server key fingerprint
  on first connect, warn on mismatch

*** TODO Issue 67: Unbounded ~recv_buf~ growth in client
:PROPERTIES:
:PRIORITY: A
:CATEGORY: security
:END:
...
**** Plan
- Track ~recv_buf~ size; if it exceeds ~max_frame_size~ (from Issue 64)
  without yielding a complete frame, disconnect with error
- Write test: client disconnects when buffer exceeds limit

** HIGH

*** TODO Issue 68: Unbounded eval input/output in server
:PROPERTIES:
:PRIORITY: A
:CATEGORY: security
:END:
...
**** Plan
- Add ~max_input_size~ (e.g. 1 MiB) check before eval
- Add ~max_output_size~ (e.g. 16 MiB) truncation on result/output strings
- Return ~Error "input too large"~ / ~Error "output truncated"~ respectively

*** TODO Issue 69: FD leak on client replacement in server
:PROPERTIES:
:PRIORITY: B
:CATEGORY: correctness
:END:
...
**** Plan
- Close existing ~client_fd~ (if any) before assigning the new one
- Write test: two successive ~accept_client~ calls don't leak fds
...
*** TODO Issue 72: Theme path traversal via ~,theme~
:PROPERTIES:
:PRIORITY: B
:CATEGORY: security
:END:
...
**** Plan
- Validate theme argument: only allow alphanumeric names (no path
  separators), resolve against a fixed themes directory
- Write test: path traversal attempt rejected
