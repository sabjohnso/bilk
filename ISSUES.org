...
*** DONE Issue 32: C FFI: ~bilk_error_message~ returns dangling pointer
:PROPERTIES:
:PRIORITY: A
:CATEGORY: security
:END:
...
**** Resolution
- Changed return type from ~const char *~ to ~char *~
- Used ~strdup(String_val(r))~ before ~CAMLreturnT~
- Updated [[file:lib/bilk.h::85][lib/bilk.h:85]] doc to say caller must ~free()~
- Added ~test_error_message_freed~ in [[file:test/test_c_embed_impl.c][test_c_embed_impl.c]]
  verifying double call + free works

*** DONE Issue 33: C FFI: ~bilk_fixnum~ truncates ~long~ to ~int~
:PROPERTIES:
:PRIORITY: A
...
**** Resolution
- Replaced ~Val_int((int)n)~ with ~Val_long(n)~ in ~bilk_fixnum~ (creation)
- Replaced ~Int_val(r)~ with ~Long_val(r)~ in ~bilk_fixnum_value~ (extraction)
  — ~Int_val~ is defined as ~((int) Long_val(x))~ which truncates to 32-bit
- Added ~test_fixnum_large~ in [[file:test/test_c_embed_impl.c][test_c_embed_impl.c]]
  verifying 3000000000L round-trips correctly

*** DONE Issue 34: C FFI: Missing ~malloc~ NULL check
:PROPERTIES:
:PRIORITY: A
:CATEGORY: correctness
:END:
...
**** Resolution
- Added ~if (argv == NULL) CAMLreturn(Val_int(0));~ after malloc
...
*** DONE Issue 70: Infinite loop in ~eval_remote~
:PROPERTIES:
:PRIORITY: B
:CATEGORY: correctness
:END:
...
**** Resolution
Added ~Recv_timeout~ exception and ~?deadline~ parameter to ~recv_msg~
using ~Unix.select~ for non-blocking wait.  ~eval_remote~ gains a
~?timeout~ parameter (default: infinity) that computes a deadline from
the current time.  Returns ~`Timeout~ if the deadline expires.
~connect~ handles the new variant by printing a message and continuing
the REPL loop.  Two new tests: timeout triggers after ~0.2s, fast
result doesn't trigger timeout.

*** DONE Issue 71: ~BILK_KEY~ cleanup incomplete
:PROPERTIES:
:PRIORITY: B
:CATEGORY: security
:END:
...
**** Resolution
- Added C stub [[file:lib/bilk_stubs.c::bilk_unsetenv][bilk_unsetenv]] calling POSIX ~unsetenv()~.
- Exposed as [[file:lib/search_path.ml::unsetenv][Search_path.unsetenv]] (natural home alongside
  other ~BILK_*~ env var handling).
- [[file:bin/main.ml::run_attach][run_attach]] now calls ~Search_path.unsetenv "BILK_KEY"~
  instead of ~Unix.putenv "BILK_KEY" ""~, fully removing the key
  from the process environment and ~/proc/*/environ~.
- Also updated [[file:test/test_search_path.ml::with_env][with_env]] test helper to use ~unsetenv~ for
  proper cleanup.
- Tests: 2 in [[file:test/test_search_path.ml][test_search_path.ml]] ~unsetenv~ group (removes
  variable, absent is noop).
...
*** DONE Issue 73: Completions count unchecked in protocol
:PROPERTIES:
:PRIORITY: B
:CATEGORY: security
:END:
...
**** Resolution
Wrapped the deserialization bodies of ~read_client_msg~ and
~read_server_msg~ in ~try/with Invalid_argument~ that converts to
~Protocol_error "truncated payload"~.  This catches all out-of-bounds
~String.sub~ accesses from ~read_bytes~, ~read_u16~, and fixed-size
extractions (auth nonces, HMACs).  Two new tests: malformed completions
count, and updated auth_response truncation test (now expects
~Protocol_error~ instead of ~Invalid_argument~).

*** DONE Issue 74: Error messages leak server file paths
:PROPERTIES:
:PRIORITY: B
:CATEGORY: security
:END:
...
**** Resolution
Added [[file:lib/repl_server.ml::sanitize_error_path][sanitize_error_path]] that strips absolute directory
prefixes from error messages, keeping only the basename (e.g.
~"/home/user/file.txt: No such file"~ → ~"file.txt: No such file"~).
Messages not starting with ~/~ are unchanged.  Applied to:
- All ~Sys_error~ handlers in comma commands (~,save-session~,
  ~,load-session~, ~,load~)
- ~Sys_error~ added to eval_loop catch list (defense in depth)
- ~Vm.Runtime_error~ and ~Failure~ in eval_loop (since primitives may
  wrap ~Sys_error~ messages in these exceptions)
Five new tests: 4 unit tests for ~sanitize_error_path~ + 1 integration
test verifying eval doesn't leak full paths.

*** DONE Issue 75: No port validation in SSH connect
:PROPERTIES:
:PRIORITY: C
:CATEGORY: correctness
:END:
...
**** Resolution
Added port range guard (~1..65535~) in ~parse_connect_line~: the ~when~
clause on the ~int_of_string_opt~ match rejects zero, negative, and
>65535 values by returning ~None~.  Five new boundary tests cover zero,
negative, >65535, and both valid endpoints (1 and 65535).

*** DONE Issue 76: ~Read_request~ prompt injection
:PROPERTIES:
:PRIORITY: C
:CATEGORY: security
:END:
...
**** Resolution
Added ~sanitize_read_prompt~ in [[file:lib/repl_client.ml][repl_client.ml]] that:
1. Prefixes all server-originated prompts with ~[remote]~ so the user
   knows the prompt comes from the server
2. Strips control characters (bell, backspace, etc.) and ANSI escape
   sequences to prevent terminal manipulation
3. Truncates to 79 characters to prevent flooding
Applied automatically in ~eval_remote~ before passing to ~on_read~.
Seven new tests cover prefix, empty, control stripping, ANSI stripping,
printable preservation, truncation, and integration.

*** DONE Issue 77: Signal handler safety
:PROPERTIES:
:PRIORITY: C
:CATEGORY: correctness
:END:
...
**** Resolution
- *Server*: Signal handler now only sets ~t.alive <- false~.  The
  ~auto_checkpoint_on_disconnect~ call is deferred to after the main
  loop exits, in normal code flow.
- *Client*: SIGINT handler now only sets ~interrupt_pending := true~.
  The Interrupt message is sent after ~eval_remote~ returns.
  ~recv_msg~ retries on EINTR via ~read_eintr~ helper instead of
  propagating EINTR as disconnection.

** LOW

*** DONE Issue 78: Custom base64 codec in repl_crypto
:PROPERTIES:
:PRIORITY: C
:CATEGORY: security
:END:
...
**** Resolution
Replaced ~75 lines of hand-rolled base64 encode/decode with the
audited ~base64~ opam package (v3.5.2).  Uses ~Base64.uri_safe_alphabet~
with ~pad:false~ to match the original URL-safe, no-padding format.
Added dependency to ~dune-project~ and ~lib/dune~.  All existing
base64 roundtrip tests pass unchanged.

*** DONE Issue 79: No forward secrecy in crypto layer
:PROPERTIES:
:PRIORITY: C
:CATEGORY: security
:END:
...
**** Resolution
Acknowledged limitation.  The SSH bootstrap (~bilk connect~) generates
a fresh key per session, so key reuse across sessions does not occur
in the primary use case.  Adding DH/ECDH ephemeral key exchange would
require a significant protocol redesign (new handshake messages, key
derivation, dependency on ~mirage-crypto-ec~) for a marginal threat
model improvement — the pre-shared key is already transported over SSH.
Documented in ~repl_crypto.mli~.

*** DONE Issue 80: No memory zeroization for key material
:PROPERTIES:
:PRIORITY: C
:CATEGORY: security
:END:
...
**** Resolution
Acknowledged limitation.  OCaml strings are immutable and managed by
the GC — there is no safe way to zero them without C stubs using
pinned memory and ~explicit_bzero~.  The threat model (local memory
dump) requires local root access, at which point the attacker can
already read the process memory directly.  Documented in
~repl_crypto.mli~.

*** DONE Issue 81: No security-focused tests for remote REPL
:PROPERTIES:
:PRIORITY: B
:CATEGORY: testing
:END:
...
**** Resolution
Added 20 adversarial tests across four test files:
- ~test_repl_protocol.ml~ (+7): unknown client/server tags, truncated
  frames, zero-length frames, short headers, offset past end, truncated
  auth_response payload
- ~test_repl_server.ml~ (+4): oversized eval rejection, garbage bytes
  handling, empty eval, null bytes in eval
- ~test_repl_client.ml~ (+3): unexpected message types during eval,
  rapid server disconnect, empty result strings
- ~test_ssh_connect.ml~ (+6): flag injection in port field, flag-like
  key accepted as opaque, extra fields rejected, empty user/host
  rejected, invalid port in scan_for_connect
